      ...
      case 'G':
        ...
        switch ( atoi( buffer ) ) {                  // Select G command                          
          case 1:
            ...
            if ( indexY <= 0 ) { // if only x was found, x changes to new position, y stays to old position
              newPos.x = atof( indexX + 1);
              newPos.y = actuatorPos.y;
            }
            else if ( indexX <= 0 ) { // if only y was found, y changes to new position, x stays to old position
...
void drawLine(float x1, float y1) {
  ...
  if (dx > dy) { //start with X-axis
    for (i = 0; i < dx; i++) {
      myStepperX.onestep(sx, STEP); //step once with on X-axis in sx direction
      ...
      if (over >= dx) { //check if the Y-axis position must be incremented
        ...
        myStepperY.onestep(sy, STEP); //step once with on Y-axis in sy direction
      }
      delay(StepDelay);
    }
  }
